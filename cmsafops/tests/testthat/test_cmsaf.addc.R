...
tempfile_nc <- function() {
  tempfile_helper("cmsaf.addc_")
}

########## output ncdf version 3 ##########
file_out <- tempfile_nc()
cmsaf.addc("SIS", 4, file.path(data_dir, "ex_normal1.nc"), file_out)
file <- nc_open(file_out)

test_that("data is correct", {
  actual <- ncvar_get(file)

  expected_data <- rep(seq(254, 276), 3)
...
test_that("attributes are correct", {
  ...
  expect_equal(actual, "cmsafops::cmsaf.addc 4 added to SIS")
...
cmsaf.addc("SIS", 4, file.path(data_dir, "ex_normal1.nc"), file_out, nc34 = 4)
...
test_that("warning is shown if var does not exist", {
  expect_warning(
    cmsaf.addc("lat", 4, file.path(data_dir, "ex_normal1.nc"), file_out),
...
test_that("warning is shown if variable is empty", {
  expect_warning(
    cmsaf.addc("", 4, file.path(data_dir, "ex_normal1.nc"), file_out),
...
test_that("error is thrown if input filename is NULL", {
  expect_error(
    cmsaf.addc("SIS", 4, NULL, file_out),
...
########## negative constant ##########
file_out <- tempfile_nc()
cmsaf.addc("SIS", -3, file.path(data_dir, "ex_normal1.nc"), file_out)
file <- nc_open(file_out)

test_that("data is correct", {
  actual <- ncvar_get(file)

  expected_data <- rep(seq(247, 269), 3)
  expected <- array(expected_data, dim = c(7, 7))
...
test_that("error is thrown if constant is NULL", {
  expect_error(cmsaf.addc("SIS", NULL,
                          file.path(data_dir, "ex_time_dim1.nc"),
                          file_out),
...
########## time length is 4 ##########
file_out <- tempfile_nc()
cmsaf.addc("SIS", 2, file.path(data_dir, "ex_time_dim3.nc"), file_out)
file <- nc_open(file_out)

test_that("data is correct", {
  actual <- ncvar_get(file)

  expected_data <- rep(seq(252, 274), 3)
...
file_out <- tempfile_nc()
cmsaf.addc("SIS", 4, file.path(data_dir, "ex_additional_attr.nc"), file_out)
file <- nc_open(file_out)
...
file_out <- tempfile_nc()
cmsaf.addc("SIS", 4, file.path(data_dir, "ex_v4_1.nc"), file_out)
file <- nc_open(file_out)
