...
test_that("attributes are correct", {
  ...
  expect_equal(actual, "cmsafops::timsum for variable SIS")
...
file_in <- file.path(data_dir, "ex_normal1.nc")
file_out <- tempfile_nc()
timsum("SIS", file_in, file_out, nc34 = 4)
file <- nc_open(file_out)
...
test_that("error is thrown if ncdf version is wrong", {
  expect_error(
    timsum("SIS", file_in, file_out, nc34 = 7),
...
test_that("ncdf version NULL throws an error", {
  expect_error(
    timsum("SIS", file_in, file_out, nc34 = NULL),
...
test_that("no error is thrown if overwrite = TRUE", {
  expect_error(
    timsum("SIS", file_in, file_out, overwrite = TRUE),
    NA
  )
})
...
test_that("data is correct", {
  ...
  temp1 <- seq(503, 541, by = 2)
  expected_data <- c(rep(c(temp1, 520, 522, 524), 2), temp1[1:3])
  expected <- array(expected_data, dim = c(7, 7))
...
test_that("data is correct", {
  ...
  temp1 <- seq(1018, 1070, by = 4)
  expected_data <- c(rep(c(temp1, 1051, 1055, 1059, 1040, 1044, 1048, 1029, 1033, 1037), 2), temp1[1:3])
  expected <- array(expected_data, dim = c(7, 7))
