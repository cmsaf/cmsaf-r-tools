...
test_that("data is correct", {
  ...
  expected_data <- c(seq(451, 463, by = 3),
                     seq(452, 464, by = 3),
                     seq(453, 465, by = 3),
                     seq(886, 898, by = 3),
                     seq(887, 899, by = 3),
...
test_that("attributes are correct", {
  ...
  expect_equal(actual, "cmsafops::monmax for variable SIS")
...
test_that("no error is thrown if var does not exist", {
  file_out <- tempfile_nc()
  expect_warning(monmax("someVariable",
                        file.path(data_dir, "ex_mon.nc"),
                        file_out),
