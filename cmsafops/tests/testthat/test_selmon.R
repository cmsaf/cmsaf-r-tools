...
test_that("attributes are correct", {
  ...
  expect_equal(actual, "cmsafops::selmon for variable SIS")
...
selmon("SIS", 1, file_in, file_out, nc34 = 4)
file <- nc_open(file_out)
...
test_that("error is thrown if ncdf version is wrong", {
  expect_error(
    selmon("SIS", 1, file_in, file_out, nc34 = 7),
...
test_that("ncdf version NULL throws an error", {
  expect_error(
    selmon("SIS", 1, file_in, file_out, nc34 = NULL),
...
test_that("error is thrown if variable is NULL", {
  expect_error(
    selmon(NULL, 1, file_in, file_out),
...
test_that("no error is thrown if overwrite = TRUE", {
  expect_error(
    selmon("SIS", 1, file_in, file_out, overwrite = TRUE),
    NA)
})
...
########## time length is 2, one month exists (first) ##########
...
########## time length is 2, one month exists (second) ##########
file_in <- file.path(data_dir, "ex_time_dim1.nc")
file_out <- tempfile_nc()
selmon("SIS", 2, file_in, file_out)
file <- nc_open(file_out)
...
########## time length is 2, both months exist ##########
file_in <- file.path(data_dir, "ex_time_dim1.nc")
file_out <- tempfile_nc()
selmon("SIS", c(1, 2), file_in, file_out)
file <- nc_open(file_out)
...
nc_close(file)
########## time length is 2, no month exists ##########
file_in <- file.path(data_dir, "ex_time_dim1.nc")
file_out <- tempfile_nc()

test_that("no error is thrown if output file already exists", {
  expect_error(
    selmon("SIS", 3, file_in, file_out),
    "No match. Months are: 1")
})
...
test_that("no error is thrown if output file already exists", {
  expect_error(
    selmon("SIS", NULL, file_in, file_out),
    "No match. Months are: 1")
})
