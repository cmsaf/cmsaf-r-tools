...
file_in <- file.path(data_dir, "ex_normal1.nc")
file_out <- tempfile_nc()
timsd("SIS", file_in, file_out, nc34 = 4)
...
test_that("error is thrown if ncdf version is wrong", {
  expect_error(
    timsd("SIS", file_in, file_out, nc34 = 7),
...
test_that("ncdf version NULL throws an error", {
  expect_error(
    timsd("SIS", file_in, file_out, nc34 = NULL),
...
test_that("error is thrown if output file already exists", {
  expect_error(
    timsd("SIS", file_in, file_out),
    paste0("File '", file_out, "' already exists. Specify 'overwrite = TRUE' if you want to overwrite it."),
    fixed = TRUE
  )
})
...
test_that("no error is thrown if overwrite = TRUE", {
  expect_error(
    timsd("SIS", file_in, file_out, overwrite = TRUE),
    NA
  )
})
...
test_that("data is correct", {
  ...
  temp1 <- 2.12132
  temp2 <- 14.14214
  expected_data <- c(rep(c(rep(temp1, 20), rep(temp2, 3)), 2), rep(temp1, 3))
  expected <- array(expected_data, dim = c(7, 7))

  expect_equivalent(round(actual, digits = 5), round(expected, digits = 5))
})
...
test_that("data is correct", {
  ...
  temp2 <- 8.845903
  temp3 <- 9.966611
  expected_data <- c(rep(c(rep(temp1, 14), rep(temp2, 3), rep(temp3, 3), rep(temp2, 3)), 2), rep(temp1, 3))
  expected <- array(expected_data, dim = c(7, 7))
...
test_that("data is correct", {
  ...
  temp1 <- 2.12132
  expected_data <- c(rep(c(rep(temp1, 19), NA, 14.14214, 14.14214, NA), 2), rep(temp1, 3))
  expected <- array(expected_data, dim = c(7, 7))
...
test_that("attributes are correct", {
  ...
  expect_equal(actual, "cmsafops::timsd for variable SIS")
