...
test_that("data is correct", {
  ...
  expected_data <- c(254, 263, 261.7778, 258, 267)
...
test_that("attributes are correct", {
  ...
  expect_equal(actual, "cmsafops::fldmean for variable SIS")
...
test_that("no error is thrown if var does not exist", {
  file_out <- tempfile_nc()
  expect_warning(fldmean("someVariable",
                         file.path(data_dir, "ex_fld.nc"),
                         file_out),
