mon_num_wrapper <- function(op, var, thld, infile, outfile, nc34, overwrite, verbose, p = NULL) {
  ...
  for (j in seq_len(test_count)) {
    ...
    switch(op,
           {
             if (verbose) message(paste0("apply monthly number above threshold ", count))
             data <- rowSums(dum_dat, dims = 2, na.rm = TRUE)
           },
           {
             if (verbose) message(paste0("apply monthly number below threshold ", count))
             data <- rowSums(dum_dat, dims = 2, na.rm = TRUE)
           },
           {
             if (verbose) message(paste0("apply monthly number equal threshold ", count))
             data <- rowSums(dum_dat, dims = 2, na.rm = TRUE)
           }
    )
