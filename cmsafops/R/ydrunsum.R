...
#'
#'The function determines multi-year daily running sum values from data of a 
#'single CM SAF NetCDF input file.
#'
#'@param var Name of NetCDF variable (character).
#'@param nts Number of consecutive timesteps. Computes running statistical values over
#'  a selected number of timesteps.
...
#'
#'@return A NetCDF file including a time series of multi-year daily running sums 
#'  is written.
#'@export
#'
...
ydrunsum <- function(var, nts = 6, infile, outfile, nc34 = 4, overwrite = FALSE, 
                     verbose = FALSE, nc = NULL) {
  ydrunx_wrapper(1, var, infile, outfile, nc34, overwrite, nts = nts, verbose = verbose, nc = nc)
}
