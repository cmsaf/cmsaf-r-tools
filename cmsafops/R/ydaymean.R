...
#'
#'The function determines multi-year daily mean values from data of a single
...
#'
#'@return A NetCDF file including a time series of multi-year daily means is
#'  written.
#'@export
#'
...
ydaymean <- function(var, infile, outfile, nc34 = 4, overwrite = FALSE, verbose = FALSE,
                     nc = NULL) {
  ...
  time_bnds <- get_time_bounds_doy(
    file_data$dimension_data$t, dateID
  )
  ...
  for (j in seq_along(unique(dateID))) {
    ...
    if (verbose) message(paste0("averaging day ", j))
