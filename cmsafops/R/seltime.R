...
#'@param hour_min Times, which should be selected, in form of a vector of
...
#'
#'@return A NetCDF file including a time series of the selected times is written.
#'@export
#'
...
seltime <- function(var, hour_min = c("00:00:00"), infile, outfile, nc34 = 4,
  ...
  hms <- get_date_time(file_data$dimension_data$t, file_data$time_info$units)$times
  if (!length(which(hour_min %in% hms))) {
    stop("No match. Times are: ", hms)
  }
