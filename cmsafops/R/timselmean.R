...
#'
#'The function determines the mean values for a pre-selected number of timesteps
#'from data of a single CM SAF NetCDF input file.
#'
#'@param var Name of NetCDF variable (character).
#'@param nts Number of input timesteps for each output timestep
...
#'
#'@return A NetCDF file including a time series of time selection means is written.
#'@export
#'
#'@family time range statistics
#'
#' @examples
...
timselmean <- function(var, nts = 6, infile, outfile, nc34 = 4, overwrite = FALSE, verbose = FALSE, nc = NULL) {
  timselx_wrapper(2, var, infile, outfile, nc34, overwrite, nts = nts, verbose = verbose, nc = nc)
}
